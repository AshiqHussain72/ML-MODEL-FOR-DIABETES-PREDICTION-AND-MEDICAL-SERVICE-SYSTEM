...
def calculate_gini(train_data, splitIndex, j, labels, leftnodeIndices):
	leftnode1_class1 = 0.0
	leftnode1_class2 = 0.0
	leftnode2_class1 = 0.0
	leftnode2_class2 = 0.0
	total_1 = 0
	total_2 = 0
	#print("splits on ", str(train_data[splitIndex,j]))
	for index in range(0, len(train_data[:,j])):
		if index in leftnodeIndices:
	...
	if(total_1 != 0 and total_2 != 0):
		eq_1 = (1 - (leftnode1_class1/float(total_1))**2 - (leftnode1_class2 / float(total_1))**2)
		eq_2 = (1 - (leftnode2_class1/float(total_2))**2 - (leftnode2_class2 / float(total_2))**2)
...
def get_split(leftnodeIndices, train_data, labels, depth):
	if(len(leftnodeIndices) == 0):
		return
	gini_value = 1.0
	splitsOn = 0
	splitOnIndex = 0
	...
	for feature in range(0, train_data.shape[1]):
		...
		for i in range(0, len(train_data)):
			if(i in leftnodeIndices):
				if train_data[i, feature] not in already_checked:
					gini_temp = calculate_gini(train_data, i, feature, labels, leftnodeIndices)
	...
	leftnode = {}
	leftnode["value"] = float(splitsOn)
	leftnode["featureIndex"] = featureIndex
	leftIndices = []
	rightIndices = []
	leftlabels = []
	rightlabels = []
	...
	leftnode["leftnode"] = checkTerminal(leftIndices, labels)

	if(leftnode["leftnode"] == -1):
		# if(len(leftlabels) < 0.05*train_data.shape[0]):
		# 	leftnode["leftnode"] = most_common(leftlabels)
		# else:
		if(len(leftIndices) == 0):
			leftnode["leftnode"] = labels[splitOnIndex]
		elif(depth == 0):
			leftnode["leftnode"] = most_common(leftlabels)
		else:
			leftnode["leftnode"] = get_split(leftIndices, train_data, labels, depth - 1)

	leftnode["rightleftnode"] = checkTerminal(rightIndices, labels)
	if(leftnode["rightleftnode"] == -1):
		# if(len(rightlabels) < 0.05*train_data.shape[0]):
		# 	leftnode["rightleftnode"] = most_common(rightlabels)
		# else:
		if(len(rightIndices) == 0):
			leftnode["rightleftnode"] = labels[splitOnIndex]
		elif(depth == 0):
			leftnode["rightleftnode"] = most_common(rightlabels)
		else:
			leftnode["rightleftnode"] = get_split(rightIndices, train_data, labels, depth - 1)
	return leftnode
...
def traverseTree(root, test_row):
	...
	if(test_row[featureIndex] < root["value"]):
		if(root["leftnode"] != 0.0 and root["leftnode"] != 1.0):
			return traverseTree(root["leftnode"], test_row)
		else:
			return root["leftnode"]
	else:
		if(root["rightleftnode"] != 0.0 and root["rightleftnode"] != 1.0):
			return traverseTree(root["rightleftnode"], test_row)
		else:
			return root["rightleftnode"]
	return -1
...
def Boosting(train_data, train_label, test_data, rounds, depth):
	leftnodeIndices = range(len(train_data))
	...
	while(n < rounds):
		...
		treeHead = get_split(leftnodeIndices, sample_data, sample_labels, depth)
		train_algo_labels = []
